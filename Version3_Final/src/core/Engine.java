...
public class Engine {
    ...
    private int hp;
    private int torque;
    private double[] gearRatio = {0, 3.2, 2.1, 1.4, 1.0, 0.8, 0.6};
    ...
    public void update(double deltaTime)
    {
        ...
        double flywheel = 0.3;
        rpm = (int) (rpm * (1 - flywheel) + targetRpm * flywheel);
    ...
    public void upShift()
    {
        if (gear < 6)
        {
            int oldGear = gear;
            gear++;
            targetRpm = Math.max((int)(rpm * (gearRatio[gear] / gearRatio[oldGear])), idle);
        }
    }
    ...
    public void downShift()
    {
        if (gear > 1)
        {
            int oldGear = gear;
            gear--;
            targetRpm = Math.min((int)(rpm * (gearRatio[gear] / gearRatio[oldGear])), redline);
        }
